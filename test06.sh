#! /usr/bin/env dash
echo '===Test edge cases of pigs-add==='
echo '---------------------------------'
...
mkdir test-dud
pigs-add test-dud >> "$OUR_OUT" 2>> "$OUR_ERR" 
pigs-add .asdf >> "$OUR_OUT" 2>> "$OUR_ERR" 
pigs-add asdf >> "$OUR_OUT" 2>> "$OUR_ERR" 
...
mkdir test-dud
2041 pigs-add test-dud >> "$REF_OUT" 2>> "$REF_ERR"
2041 pigs-add .asdf >> "$REF_OUT" 2>> "$REF_ERR"
2041 pigs-add asdf >> "$REF_OUT" 2>> "$REF_ERR"
