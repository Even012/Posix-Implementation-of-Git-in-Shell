...
echo '=== Test edge cases of pigs-commit ==='
...
pigs-commit -b -m 'test' >> "$OUR_OUT" 2>> "$OUR_ERR"
pigs-commit -m ''  >> "$OUR_OUT" 2>> "$OUR_ERR"
pigs-commit -m -a 'test' >> "$OUR_OUT" 2>> "$OUR_ERR"
pigs-commit -b -m 'test' >> "$OUR_OUT" 2>> "$OUR_ERR"
pigs-commit -m 'test'  >> "$OUR_OUT" 2>> "$OUR_ERR"
...
2041 pigs-commit -b -m 'test' >> "$REF_OUT" 2>> "$REF_ERR"  
2041 pigs-commit -m '' >> "$REF_OUT" 2>> "$REF_ERR" 
2041 pigs-commit -m -a 'test' >> "$REF_OUT" 2>> "$REF_ERR" 
2041 pigs-commit -b -m 'test' >> "$REF_OUT" 2>> "$REF_ERR" 
2041 pigs-commit -m 'test' >> "$REF_OUT" 2>> "$REF_ERR" 
